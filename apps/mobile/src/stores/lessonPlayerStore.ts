...
export interface LessonPlayerState {
  ...
  // No OS-level fullscreen API on mobile -- this instead toggles a "focus mode" layout that
  // hides the side panels and lets the visualizer fill the screen (see LessonPlayer.tsx).
  isFocusMode: boolean;
  actions: {
    play: () => void;
    pause: () => void;
  ...
  return createStore<LessonPlayerState>((set, get) => ({
    ...
    actions: {
      ...
      setPendingInteraction: (interaction) =>
        set((s) => ({ pendingInteraction: interaction, isPlaying: interaction ? false : s.isPlaying })),
